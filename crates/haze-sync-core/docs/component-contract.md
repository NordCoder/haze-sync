...
- Stored idempotency responses must use a final `200..=599` status, safe public JSON, normalized replay headers with visible ASCII values, and no credentials, cookies, idempotency keys, hop-by-hop headers, or payload-framing headers.
- Changes pages must contain no more rows than the applicable query/global limit, and `has_more = true` must accompany sequence progress.
...
- Stored idempotency responses are persistence snapshots, not general HTTP archives; transport/framing headers are regenerated by the replaying HTTP layer.
- Operation-log and changes-feed outputs validate sequence ordering, limit bounds, page progress, and cursor monotonicity but do not query or mutate storage.
...
- idempotency key validation, request fingerprint determinism, replay of same request, conflict for different request, final-status bounds, and replay-header safety;
- changes query bounds, monotonic changes pages, page limit/progress validation, operation kind parsing, and cursor regression rejection;
