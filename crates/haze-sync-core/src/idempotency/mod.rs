...
//!
//! The types in this module model retry-safety metadata only. They never store
//! raw request bodies, file bytes, bearer tokens, provider payloads, database
//! URLs, filesystem paths outside public vault metadata, or runtime details.

use haze_sync_common::{AdapterId, Sha256, ValidationError};
...
impl IdempotencyKey {
    ...
    pub fn parse(input: &str) -> Result<Self, IdempotencyError> {
        ...
        if input.len() > MAX_IDEMPOTENCY_KEY_LEN
            || input
                .as_bytes()
                .iter()
...
impl RequestFingerprint {
    ...
    ///
    /// Callers must pass only metadata such as route, path, base revision, content
    /// hash, and size. Raw file bytes and request bodies must not be included.
    #[must_use]
    ...
    ///
    /// This is intended for small canonical metadata envelopes, not raw file
    /// contents or sensitive request bodies.
    #[must_use]
...
impl StoredIdempotencyRecord {
    ...
    /// Returns the request fingerprint stored with this key.
    #[must_use]
    pub const fn request_fingerprint(&self) -> &RequestFingerprint {
        &self.request_fingerprint
    }

    /// Returns the safe response snapshot for replay.
    #[must_use]
...
pub enum IdempotencyReplayOutcome {
    /// No existing idempotency record was found; caller may process the write.
    NewRequest,
    /// Existing key and request fingerprint match; replay the stored response.
    ReplaySameRequest {
        /// Previously stored safe response.
        response: StoredIdempotencyResponse,
    },
    /// Existing key was reused for a different request fingerprint.
    ConflictDifferentRequest {
        /// Stored request fingerprint for this key.
        stored_request_fingerprint: RequestFingerprint,
        /// Incoming request fingerprint.
...
pub enum IdempotencyError {
    /// The idempotency key is empty, too long, or not safe for a header value.
    InvalidKey,
    /// The request fingerprint is not a valid SHA-256 value.
    InvalidRequestFingerprint,
    /// The stored response metadata is not safe to persist or replay.
    InvalidStoredResponse,
    /// A replay header name is invalid or sensitive.
...
impl From<ValidationError> for IdempotencyError {
...
mod tests {
    ...
    fn same_key_same_fingerprint_returns_replay() {
        ...
        let outcome = IdempotencyService::evaluate(Some(&stored), fingerprint);
    ...
    fn same_key_different_fingerprint_returns_conflict() {
        ...
        let outcome = IdempotencyService::evaluate(Some(&stored), incoming_fingerprint);
    ...
    fn sensitive_headers_are_rejected() {
        ...
        assert_eq!(
            StoredIdempotencyResponse::new(200, json!({ "ok": true }), headers).unwrap_err(),
            IdempotencyError::UnsafeResponseHeader
        );
    }

    #[test]
    fn fingerprint_uses_canonical_json_key_order() {
