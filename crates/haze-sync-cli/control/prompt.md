...
Expected scope is rustfmt-equivalent formatting only in cli-owned Rust files.

## Allowed files

- crates/haze-sync-cli/**/*.rs
- crates/haze-sync-cli/control/report.md
...
If the diagnostics reveal a non-formatting issue, fix it only if it is inside cli scope and required for CI. Otherwise report the appropriate blocker.
