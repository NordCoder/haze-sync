...
- a final HTTP status code in the inclusive range `200..=599`;
- validated replay headers with normalized lowercase names and visible ASCII values;
- a public JSON body.
...
Core also rejects hop-by-hop and payload-framing headers such as `connection`,
`content-length`, `content-encoding`, `transfer-encoding`, `trailer`, and
`upgrade`, because those values must be regenerated by the replaying HTTP layer.
...
- never set `has_more = true` for an empty page, because pagination must advance;
- use the final returned row as `to_seq`, or `from_seq` for an empty page.

`ChangesPage::new` enforces the requested limit and rejects non-advancing
`has_more` pages. Deserialization cannot recover the original requested limit, so
it enforces the global `MAX_CHANGES_LIMIT` together with ordering, bounds, and
progress invariants.
